...
namespace BezierSolution
{
	public static class BezierUtils
	{
		...
		public static void DrawSplineInspectorGUI( BezierSpline spline )
		{
            ...
            EditorGUI.BeginChangeCheck();
            PATHTYPE pathType = (PATHTYPE)EditorGUILayout.EnumPopup(spline.pathType);
            if (EditorGUI.EndChangeCheck())
            {
                spline.pathType = pathType;
                SceneView.RepaintAll();
            }
            EditorGUI.BeginChangeCheck();
            bool isTunnel = EditorGUILayout.Toggle("isTunnel", spline.isTunnel);
            if (EditorGUI.EndChangeCheck())
            {
                spline.isTunnel = isTunnel;
                SceneView.RepaintAll();
            }

            EditorGUI.BeginChangeCheck();
            bool isDestroyble = EditorGUILayout.Toggle("isDestroyble", spline.isDestroyble);
            if (EditorGUI.EndChangeCheck())
            {
                spline.isDestroyble = isDestroyble;
                SceneView.RepaintAll();
            }
            ...
            EditorGUI.BeginChangeCheck();
            bool isYjun = EditorGUILayout.Toggle("isYjun", spline.isYjun);
            if (EditorGUI.EndChangeCheck())
            {
                spline.isYjun = isYjun;
                SceneView.RepaintAll();
            }

            EditorGUI.BeginChangeCheck();
            string mapNo = EditorGUILayout.TextField("mapNo","0");
            if (EditorGUI.EndChangeCheck())
            {
                spline.mapNo = int.Parse(mapNo);
                SceneView.RepaintAll();
            }
